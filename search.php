<html lang="en">
	...
	<body background="galaxy.jpg">
		...
		<?php
			...
			$origSearch = $mysqli->real_escape_string($_POST["search"]);
			
			if ($mysqli->connect_errno) {
				echo('could not connect to database');
			}//if
			
			//search first title, then author, then keywords and add the results in that order
			// Performing SQL query
			$query = 	"SELECT DISTINCT Title, Author, Cover, SentimentScore, NumStars FROM book WHERE Title LIKE '%" . $origSearch . "%' GROUP BY Title" . 
						" UNION SELECT Title, Author, Cover, SentimentScore, NumStars FROM book WHERE Author LIKE '%" . $origSearch . "%' GROUP BY Title" . 
						" UNION SELECT Title, Author, Cover, SentimentScore, NumStars FROM book b, keyword k WHERE";
			
			$search = explode(" ", $origSearch);
			...
			foreach($search as $word){
				...
				$query .= " k.Word LIKE '%" . $word . "%'";
				$i++;
			}

			$query .= " AND k.Bookid = b.Id GROUP BY k.Num";
			...
			if($result->num_rows > 0){
				...
				echo '<h3>' . $result->num_rows . ' Results matching ' . $origSearch . ':</h3>';
				...
				while ($book = $result->fetch_assoc()) {
					...
					$sentimentText = round($book['SentimentScore'] * 100, 2);
					if($sentimentScore > 0){
				...
				echo '<h3>No results found matching ' . $search . '.</h3>';
			}
